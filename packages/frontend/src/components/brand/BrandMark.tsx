  ...
  const CustomLogo =
    meta.customLogo === 'opencode'
      ...
      : meta.customLogo === 'commandcode'
        ? CommandCodeLogo
        : null;
  const isBrandedMark = Boolean(logo || CustomLogo);
  const sizeClass =
    size === 'sm'
      ...
      <span
        className={cn(
          ...
          isBrandedMark
            ...
            : 'border border-border bg-surface-elevated text-white shadow-[var(--shadow-card)]',
          sizeClass,
          className,
        )}
        style={{
          color: isBrandedMark ? undefined : '#fff',
          backgroundColor: isBrandedMark ? undefined : meta.color,
        }}
      >
        {CustomLogo ? (
...
function OpenCodeLogo(props: SVGProps<SVGSVGElement>) {
  return (
    <svg viewBox="0 0 512 512" fill="none" aria-hidden="true" {...props}>
      <path d="M320 224V352H192V224H320Z" fill="currentColor" opacity="0.3" />
      <path
        fillRule="evenodd"
        clipRule="evenodd"
  ...
  return (
    <Badge variant="neutral" className="gap-1.5 py-1 pr-2.5">
      <BrandMark
        ...
        kind={kind}
        size="sm"
        className="h-4 w-4 rounded-full border-0 text-[7px] shadow-none"
      />
      {meta.label}
    </Badge>
  );
