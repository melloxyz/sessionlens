...
const ANTHROPIC = { light: 'https://svgl.app/library/anthropic_black.svg', dark: 'https://svgl.app/library/anthropic_white.svg' };
...
const CLI_BRANDS: Record<string, BrandMeta> = {
  ...
  qwen: { label: 'Qwen CLI', initials: 'QW', color: '#615ced', logo: { light: 'https://svgl.app/library/qwen_light.svg', dark: 'https://svgl.app/library/qwen_dark.svg' } },
  antigravity: { label: 'Antigravity', initials: 'AG', color: '#f59e0b', logo: 'https://svgl.app/library/antigravity.svg' },
};
...
export function getBrandMeta(value: string | null | undefined, kind: 'cli' | 'provider' = 'cli'): BrandMeta {
...
export function BrandMark({ value, kind = 'cli', size = 'md', className }: { value: string | null | undefined; kind?: 'cli' | 'provider'; size?: 'sm' | 'md' | 'lg'; className?: string }) {
...
export function BrandBadge({ value, kind = 'cli' }: { value: string | null | undefined; kind?: 'cli' | 'provider' }) {
  const meta = getBrandMeta(value, kind);

  return (
    <Badge variant="neutral" className="gap-1.5 py-1 pr-2.5">
      <BrandMark value={value} kind={kind} size="sm" className="h-4 w-4 rounded-[5px] border-0 text-[7px] shadow-none" />
      {meta.label}
    </Badge>
  );
}
