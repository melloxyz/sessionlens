  ...
  return (
    <div className="grid gap-5 p-6 xl:grid-cols-[minmax(0,1fr)_380px]">
                ...
                {adapterRows.length > 0 && (
                  <div className="grid gap-2 sm:grid-cols-2">
                    {adapterRows.map(([cli, info]) => (
                      <div
                        key={cli}
                        className="flex items-center justify-between gap-3 rounded-2xl border border-border bg-surface-elevated p-3"
                      >
      ...
      <aside className="space-y-5">
        <Card>
          <CardHeader>
            <CardTitle>{t('settings.workspaceSummary')}</CardTitle>
          </CardHeader>
          <CardContent className="space-y-4">
            {overviewError && (
              <ErrorState
        ...
        <Card>
          ...
          <CardContent className="space-y-3">
            {(integrations?.integrations ?? []).map((item) => (
              <div
                key={item.cli}
                className="flex items-center justify-between rounded-2xl border border-border bg-surface-muted p-3"
              >
...
function PreferenceButton({
  ...
  return (
    <button
      onClick={onClick}
      className={`flex items-start justify-between gap-4 rounded-2xl border p-4 text-left transition-all hover:bg-surface-hover ${active ? 'border-accent bg-accent-soft' : 'border-border bg-surface'}`}
    >
      <div>
...
function StatusTile({ label, value }: { label: string; value: string }) {
  return (
    <div className="rounded-2xl border border-border bg-surface-muted p-4">
      <div className="text-[11px] uppercase tracking-[0.14em] text-subtle-foreground">{label}</div>
      <div className="mt-1 text-2xl font-semibold tracking-[-0.05em] text-foreground">{value}</div>
    </div>
  );
}
